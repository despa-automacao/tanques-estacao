...
uint8_t sensor1[8] = { 0x28, 0xBF, 0xD2, 0x95, 0xF0, 0x01, 0x3C, 0xD4 }; //tanque
uint8_t sensor2[8] = { 0x28, 0x44, 0x97, 0x95, 0xF0, 0x01, 0x3C, 0x65 }; //tanque
uint8_t sensor3[8] = { 0x28, 0x0F, 0x5B, 0x95, 0xF0, 0x01, 0x3C, 0xEB }; //tanque
uint8_t sensor4[8] = { 0x28, 0x02, 0x71, 0x95, 0xF0, 0x01, 0x3C, 0xE4 }; //torre
uint8_t sensor5[8] = { 0x28, 0x3E, 0x4E, 0x95, 0xF0, 0x01, 0x3C, 0x9F }; //torre
...
void setup() {
  ...
  for (int i = 0; i < 5; i) {    // ************ iniciando sensores ++++++++++
    if (millis() - salva_leitura_temperaturas > 1000) {
      ...
      valor_sensor1 = 0.001 * valor_exponencial + 0.9173 * modulo_sensor; //curva de calibração Tdurae _ T1.1 data 11/08/22
      modulo_sensor = abs(temp2_tanque);
      valor_exponencial = pow(modulo_sensor, expoente);
      valor_sensor2 = 0.0006 * valor_exponencial + 0.9409 * modulo_sensor; //curva de calibração Tdurae _ T1.2 data 11/08/22
      modulo_sensor = abs(temp3_tanque);
      valor_exponencial = pow(modulo_sensor, expoente);
      valor_sensor3 = 1.0478 * modulo_sensor - 2, 31 - 0.0002 * valor_exponencial; //curva de calibração Tdurae _ T1.3 data 11/08/22
      temp_media_tanque = (valor_sensor1 + valor_sensor2 + valor_sensor3) / 3;
...
void loop() {
  ...
  //if(estado_sobe_temp == true) Serial.println("Estado sobe temp");
  //if(temp_media_tanque > setpoint_temp) Serial.println("Condicao temp");
  if (estado_sobe_temp == true && temp_media_tanque > setpoint_temp + 0.5 ) { //chegando na temp atravez da resistencia
    ...
    digitalWrite(VALV_TORRE, LOW);
    digitalWrite(RESISTENCIA, LOW);
    //Serial.println("RESISTENCIA LOW \nRESISTENCIA LOW \nRESISTENCIA LOW \nRESISTENCIA LOW \n ");
  }
  ...
  if (millis() - salva_att_tela > 300) {
    if ((aviso_temp_alta_display == false) || (aviso_temp_baixa_display == false) ) {
      if (tela_programacao == false && tela_esvazia == false) {
        if (temp_backup_torre_final <= 0 && temp_backup_tanque_final <= 10 ) {
          lcd.clear();
          lcd.setCursor(0, 0);
        ...
        if (temp_backup_torre_final > 0 && temp_backup_tanque_final <= 10 ) {
          ...
          lcd.print("T TORRE: ");
          lcd.setCursor(11, 0);
          lcd.print(temp_backup_torre_final);
          lcd.setCursor(0, 1);
          lcd.print("comunicando");
        }
        if (temp_backup_torre_final <= 0 && temp_backup_tanque_final > 0 ) {
          lcd.clear();
          lcd.setCursor(0, 0);
          lcd.print("comunicando");
          lcd.setCursor(0, 1);
          lcd.print("T TANQUE: ");
          lcd.setCursor(11, 1);
          lcd.print(temp_backup_tanque_final);
        }
        if (temp_backup_torre_final > 0 && temp_backup_tanque_final > 0 ) {
          lcd.clear();
          lcd.setCursor(0, 0);
  ...
  if (controle_automatico_bombas == true) {                // --------------------- bomba -------------------------- autoo -------------------------------
    ...
    if (ESTADO_LIQUIDO == 2) { //l circula agua
      ...
      if (fluxo_bomba == 1 && estado_setpoint == true) {
        ...
        digitalWrite(VALV_TAM, HIGH);
        digitalWrite(AGUA_RUA, LOW);
        if (millis() - verificador_5_minutos_nivel_alto >= 330000) {
        }
        else {
          digitalWrite(SIRENE, LOW);
        }
        if ((millis() - verificador_5_minutos_nivel_alto >= 25000) && estado_setpoint == true) { //1,5 minutos tanque 2
          digitalWrite(BOMBA_TORRE, LOW);
          digitalWrite(VALV_TORRE, LOW);
        }
        else {
  ...
  if (millis() - escreve_cartao > 300000 && maquina_sem_cartao_sd == false) {//faz a leitura na serial e salvamento no SD a cada 60 segundo
  ...
  if (millis() - tempo_para_ler_primeiro > 120000) {
    if (temp_backup_torre_final >= 105) { // desliga limite minimo de temp
      lcd.clear();
      lcd.setCursor(0, 0);
    ...
    if (temp_backup_torre_final <= 5) { // desliga limite minimo de temp
